...
tag by category, and enrich with parent_asin from local books.db (title_author_key
lookup). Optionally can use backend storage (DynamoDB) if available. Outputs JSON
for pipeline and DynamoDB loader. Run locally first so everything works without cloud.
"""
...
def _load_books_by_title_author_key(db_path: str) -> dict[str, dict]:
    """
    Load from local books.db a map title_author_key -> {parent_asin, categories}.
    Uses the same title_author_key column as books_meta_data. Returns empty dict if
    db_path does not exist or table is missing.
    """
    out = {}
    path = Path(db_path)
    if not path.exists():
        ...
        with sqlite3.connect(path) as conn:
            ...
            for row in cur:
                ...
                if isinstance(cats, str) and cats:
                    ...
                    cats = []
                out[key] = {"parent_asin": row["parent_asin"], "categories": cats}
...
def clean_events(df: pd.DataFrame) -> pd.DataFrame:  # pylint: disable=too-many-branches
    ...
    # Map events to book parent_asin using local books.db (no persistent map file).
    # Build in-memory lookup title_author_key -> {parent_asin, categories}; use it to set
    # parent_asin (and merge book categories into tags) on each event. Only the event
    # records are persisted (each has parent_asin and title_author_key on the output).
    books_map = _load_books_by_title_author_key(BOOKS_DB_PATH)
    get_book_from_storage = getattr(_storage, "get_book_by_title_author_key", None)
            ...
            if book:
                ...
                for cat in book.get("categories") or []:
                    if cat and cat not in tags:
                        tags.append(cat)
            elif get_book_from_storage:
                try:
                    book = get_book_from_storage(key)
                    if book:
                        parent_asin = book.get("parent_asin")
                        for cat in book.get("categories") or []:
                            if cat and cat not in tags:
