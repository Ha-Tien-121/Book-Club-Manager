...
Usage:
    python -m data.scripts.load_books_to_dynamodb [--limit N]
    python -m data.scripts.load_books_to_dynamodb --all
    python -m data.scripts.load_books_to_dynamodb --all --create-table
...
def row_to_item(row: sqlite3.Row) -> dict:
    """Convert SQLite row to DynamoDB item (excludes description)."""
    item = {
