"""App entrypoint implementation (called by streamlit_app.py)."""
...
from app.load_data.loader import load_data
from app.pages.auth import auth_panel
from app.stores.forum_store import load_forum_store, save_forum_store
...
from app.ui.components import render_book_card, render_book_carousel, render_pill_tags
...
def render_book_detail_page(
    *,
    ...
    """Render a simple Book Detail page with library save action."""
...
def main() -> None:
            ...
            for key, tab in zip(["saved", "in_progress", "finished"], ltabs):
                with tab:
                    ...
                    for i, bid in enumerate(book_ids):
                        with cols[i % 3]:
                            render_book_carousel(
                                section_key=f"lib_{key}_{bid}",
                                books=[books_by_id[bid]],
                                cards_per_page=1,
    ...
    with tabs[4]:
        ...
        view = st.radio("View", ["All", "Public", "Club Discussions", "Saved"], horizontal=True)
...
def main() -> None:
    ...
    with tabs[0]:
        ...
        selected_genres = st.multiselect(
            "Filter by genre", genres, key="feed_genre_filter"
        )
        filtered = [
            b
            for b in books
        ...
        if RECOMMENDER_AVAILABLE:
            try:
                if st.session_state.get("signed_in") and current_user is not None:
                    user_email = st.session_state.get("user_email", "")
                    user_genres_store, user_books_store, has_user_data = (
                        build_user_recommender_stores(
                            current_user=current_user,
                            user_email=user_email,
                            books_by_id=books_by_id,
                            clubs=clubs,
        ...
        if selected_genres:
            allowed = {g.lower() for g in selected_genres}
            clubs_source = [c for c in clubs_source if c.get("genre", "").lower() in allowed]
        top_clubs = clubs_source[:5]
        if not top_clubs:
            st.caption("No suggested clubs for this filter.")
        for club in top_clubs:
    ...
    with tabs[1]:
        ...
        if nfilter != "All":
            filtered_clubs = [
                c for c in filtered_clubs if nfilter.lower() in c["location"].lower()
            ]
        for club in filtered_clubs:
            st.subheader(club["name"])
            st.caption(f"{club['genre']} | {club['location']}")
    ...
    with tabs[3]:
        ...
        if not st.session_state.get("signed_in") or current_user is None:
            ...
            books_by_id_local = books_by_id
            user_library = current_user["library"]
            ltabs = st.tabs(["Saved", "In Progress", "Finished"])
            for key, tab in zip(["saved", "in_progress", "finished"], ltabs):
                with tab:
                    book_ids = [bid for bid in user_library[key] if bid in books_by_id_local]
                    if not book_ids:
                        st.caption("No books in this list yet.")
                        continue
                    cols = st.columns(3)
                    for i, bid in enumerate(book_ids):
                        with cols[i % 3]:
                            # Render minimal card (reuse carousel rendering via 1-item list)
                            render_book_carousel(
                                section_key=f"lib_{key}_{bid}",
                                books=[books_by_id_local[bid]],
                                cards_per_page=1,
                                key_prefix=f"lib_{key}_{i}",
                                auth_user=st.session_state.get("user_email", ""),
                            )
...
def render_book_detail_page(
    ...
    for post in forum_posts_data:
        ...
        tag_hit = any(
            book_title_lower == t.strip().lower() for t in post.get("tags", [])
        )
    ...
    with st.form(f"book_post_form_{book['id']}"):
        ...
        if visibility == "Club Members":
            ...
            selected_club_name = c2.selectbox(
                "Club",
                [c.split("::", 1)[1] for c in club_options]
                if club_options
...
def main() -> None:
    ...
    with tabs[0]:
        ...
        if selected_genres:
            clubs_source = [
                c
                for c in clubs_source
                if c.get("genre", "").lower()
                in {g.lower() for g in selected_genres}
            ]
        top_clubs = clubs_source[:5]
        if not top_clubs:
            st.caption("No suggested clubs for this filter.")
        for club in top_clubs:
            ...
            st.write(
                (club.get("description", "") or "")[:180]
                + ("..." if len(club.get("description", "")) > 180 else "")
            )
            if club.get("external_link"):
                st.link_button("Open club", club["external_link"], use_container_width=False)
            st.divider()
    ...
    with tabs[4]:
        ...
        if st.session_state.get("selected_forum_post_id") is not None:
            selected_post = next(
                (
                    ...
                    if int(p.get("id", -1)) == int(st.session_state["selected_forum_post_id"])
                ),
                None,
            )
            if selected_post is not None and can_view_forum_post(selected_post, current_user):
                ...
                if current_user is not None:
                    ...
                    if a2.button("Unsave post" if is_saved else "Save post", key=f"save_post_{selected_post['id']}"):
                        if is_saved:
                            current_user["saved_forum_post_ids"] = [pid for pid in saved_ids if int(pid) != int(selected_post["id"])]
                        else:
        ...
        if st.session_state.get("selected_forum_post_id") is None:
                ...
                if submitted:
                    if post_title.strip() and post_text.strip():
                        if visibility == "Club Members" and not joined_clubs:
                            ...
                            forum_store["next_post_id"] = (
                                int(forum_store["next_post_id"]) + 1
                            )
                            save_forum_store(forum_store)
            ...
            for post in posts:
                ...
                st.write(post["preview"])
                st.caption(
                    f"Likes: {int(post.get('likes',0))} | Replies: {int(post.get('replies',0))}"
                )
                if st.button(
                    "Open discussion",
                    key=f"open_forum_post_{int(post['id'])}",
                ):
                    st.session_state["selected_forum_post_id"] = int(post["id"])
                    st.rerun()
                st.divider()
